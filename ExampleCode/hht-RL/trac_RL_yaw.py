'''test the trained hht-RL model with GUI for yawing'''
...
class testRLtrac():
    def __init__(self):
        # self.start_pos_xy=np.array([0.2,0,0.5])
        # self.start_pos_xz=np.array([0,0,0.52])
        self.start_pos_yaw=np.array([0,0,0.5])
        # self.initial_pos=self.start_pos_xz
        # self.initial_att=np.array([0,0,0])
        # self.target_pos=self.start_pos_xz
        # self.trac_pos=self.start_pos_xz
        self.initial_pos=self.start_pos_yaw
        self.initial_att=np.array([0,0,0])
        self.target_pos=self.start_pos_yaw
        self.trac_pos=self.start_pos_yaw
        self.target_att=np.array([0,0,0])
        self.trac_att=np.array([0,0,0])
    ...
    def test(self):
        ...
        print('i=0,obs:')
        obs, info = test_env.reset(self.trac_pos,self.trac_att)
        print(obs[0:12])
        start = time.time()

        whole_ctrl_steps = int(self.real_t*int(test_env.CTRL_FREQ))
        up_cnt = 24
        #every up_pos ctrlsteps update trac_pos
        up_pos=int(whole_ctrl_steps/up_cnt)
        ...
        for i in range(whole_ctrl_steps):
            ...
            if(((i+up_pos)%up_pos)==0):
                # self.trac_pos = self.Circle_trac_xz((i+1.4*up_pos)/whole_ctrl_steps)
                self.trac_att = self.trac_att + np.array([0,0,(1/up_cnt)*np.pi])
            obs, reward, terminated, truncated, info = test_env.step(action,trac_pos=self.trac_pos,trac_att=self.trac_att)
            # if (i==1):
            #     print('i=1,obs:')
            #     print(obs[0:12])
            #     time.sleep(5)
            # if (i==3
            #     or i==2*int(test_env.CTRL_FREQ) 
            #     or i==(4*int(test_env.CTRL_FREQ)+3)
            #     or i==(6*int(test_env.CTRL_FREQ)+5)
            #     or i==(8*int(test_env.CTRL_FREQ)+7)
            #     or i==(10*int(test_env.CTRL_FREQ)-3)):
            #     time.sleep(5)
            # if (i==300):
    ...
    def Circle_trac_xy(self,timefrac):
        R=0.2
        theta = timefrac * 2 * np.pi  
        x = -0.02+0.95*(R * np.cos(theta)-2*R* np.sin(theta))  
        y = 1.3*(R * np.sin(theta)+2*R* np.cos(theta)) 
        z = self.initial_pos[2]  
        return np.array([x, y, z])
    
    def Circle_trac_xz(self,timefrac):
        R=0.2
        theta = timefrac * 2 * np.pi+0.5*np.pi  
        # x = 2*R * np.cos(theta)-1.5*R * np.sin(theta)
        # y = self.initial_pos[1] 
        # z = 0.32+1.1*R * np.sin(theta)+0.5*R * np.cos(theta)
        x = 1.8*R * np.cos(theta)-1.5*R * np.sin(theta)
        y = self.initial_pos[1] 
        z = 0.32+1.1*R * np.sin(theta)+0.5*R * np.cos(theta) 
        # x = 2*R * np.cos(theta)  
        # y = self.initial_pos[1] 
        # z = 0.3+1.1*R * np.sin(theta)
        return np.array([x, y, z])
    ...
    def clip_yaw(self):
        j=0
        yaw_array = self.states[j, 5, :]
        yaw_array[yaw_array<-np.pi/2] += 2*np.pi
        self.states[j, 5, :]=yaw_array
    ...
    def plot_xyz(self):
        ...
        # axs.set_xticks(np.arange(0, 3,0.5))
        ...
        axs.legend(handles, labels, loc='upper right', fontsize=alp_size+2)
    ...
    def plot_rpy(self):
        ...
        # axs.set_xticks(np.arange(0, 11,1))
        axs.set_xticks(np.arange(0, 3,0.5))
        axs.set_yticks(np.arange(-10, 200, 10))
        axs2.set_yticks(np.arange(-10, 200, 10))
        axs3.set_yticks(np.arange(-10, 200, 10))
        axs.tick_params(axis='x', labelsize=tick_size)
        axs.tick_params(axis='y', labelsize=tick_size)
        axs2.tick_params(axis='y', labelsize=tick_size)
        axs3.tick_params(axis='y', labelsize=tick_size)
    ...
    def plot_yaw(self):
        ...
        handle, = axs.plot(t_real, (180/np.pi)*(self.states[j, 5, :]), linewidth=0.5, label="yaw", color='tab:blue')
        handles.append(handle)
        labels.append("yaw")
