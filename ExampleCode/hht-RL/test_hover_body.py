'''Test the trained hht-RL model with GUI for hovering, using RLattbody.'''
...
class testRLhover():
    ...
    def test(self):
        ...
        path = DEFAULT_OUTPUT_FOLDER+'/save-07.09.2024_21.34.57/best_model.zip'
        
        model = PPO.load(path)

        test_env = RLattbody(gui=True,
                         trunc_flag=True,
    ...
    def plot(self):
        ...
        row = 4
        for j in range(self.num_mavs):
            axs[row, col].plot(t, np.pi  * self.states[j, 4, :], )
        axs[row, col].set_xlabel('time')
