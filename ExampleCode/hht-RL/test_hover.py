'''Test the trained hht-RL model with GUI for hovering.'''
...
class testRLhover():
    def __init__(self):
        # self.initial_pos=np.array([0,0,0.2])
        # self.initial_att=np.array([0,0,np.pi/4])
        # self.target_pos=np.array([0.5,0.5,0.7])
        # self.target_att=np.array([0,0,np.pi/4])
        self.initial_pos=np.array([0,0,0.5])
        self.initial_att=np.array([0,0,0])
        self.target_pos=np.array([0,0,0.5])
        # self.target_att=np.array([0,0,(90/180)*np.pi])
    ...
    def test(self):
        ...
        # path = DEFAULT_OUTPUT_FOLDER+'/save-11.23.2024_11.00.12/best_model.zip'
        path = DEFAULT_OUTPUT_FOLDER+'/save-05.06.2024_20.24.09/best_model.zip'
        # path = DEFAULT_OUTPUT_FOLDER+'/save-11.30.2024_22.31.53/best_model.zip'
        ...
        # for i in range(2000):
        #     test_env.pause()
        for i in range(int(self.real_t*test_env.CTRL_FREQ)):
            ...
            if (i==1):
                print('i=1,obs:')
                print(obs[0:12])
            # if (i==2*int(test_env.CTRL_FREQ) 
            #     or i==(5*int(test_env.CTRL_FREQ)+7)
            #     or i==(10*int(test_env.CTRL_FREQ)+7)):
            #     time.sleep(5)
                # test_env.erase_trac()
            # if (i==300):
            ...
            state = np.hstack([true_xyzrpy[0:6], obs[6:12],action])
    ...
    def plot(self):
        ...
        row = 3
        for j in range(self.num_mavs):
            axs[row, col].plot(t, (180/np.pi)* self.states[j, 3, :], )
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('r (deg)')
        row = 4
        for j in range(self.num_mavs):
            axs[row, col].plot(t, (180/np.pi)* self.states[j, 4, :], )
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('p (deg)')
        row = 5
        for j in range(self.num_mavs):
            axs[row, col].plot(t, (180/np.pi)* self.states[j, 5, :], )
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('y (deg)')
    ...
    def plot_xyzrpy(self):
        ...
        row = 0
        for j in range(self.num_mavs):
            axs[row].plot(t_real,  self.states[j, 0, :]*1000,linewidth=0.5)
        axs[row].set_xlabel('time(s)',loc='right', fontsize=alp_size)
        axs[row].set_ylabel('x (mm)', fontsize=alp_size)
        axs[row].tick_params(axis='x', labelsize=tick_size )
        axs[row].tick_params(axis='y', labelsize=tick_size )

        row = 1
        for j in range(self.num_mavs):
            axs[row].plot(t_real, self.states[j, 1, :]*1000 )
        axs[row].set_xlabel('time(s)',loc='right', fontsize=alp_size)
        axs[row].set_ylabel('y (mm)', fontsize=alp_size)
        axs[row].tick_params(axis='x', labelsize=tick_size )
        axs[row].tick_params(axis='y', labelsize=tick_size )

        row = 2
        for j in range(self.num_mavs):
            axs[row].plot(t_real,  self.states[j, 2, :]*1000)
        axs[row].set_xlabel('time(s)',loc='right', fontsize=alp_size)
        axs[row].set_ylabel('z (mm)', fontsize=alp_size)
        axs[row].tick_params(axis='x', labelsize=tick_size )
        axs[row].tick_params(axis='y', labelsize=tick_size )
        ...
        row = 3
        for j in range(self.num_mavs):
            axs[row].plot(t_real, (180/np.pi)*self.states[j, 1, :] )
        axs[row].set_xlabel('time(s)',loc='right', fontsize=alp_size)
        axs[row].set_ylabel('roll (deg)', fontsize=alp_size-2)
        axs[row].tick_params(axis='x', labelsize=tick_size )
        axs[row].tick_params(axis='y', labelsize=tick_size )

        row = 4
        for j in range(self.num_mavs):
            axs[row].plot(t_real,  (180/np.pi)*self.states[j, 4, :],linewidth=0.5)
        axs[row].set_xlabel('time(s)',loc='right', fontsize=alp_size)
        axs[row].set_ylabel('pitch (deg)', fontsize=alp_size-2)
        axs[row].tick_params(axis='x', labelsize=tick_size )
        axs[row].tick_params(axis='y', labelsize=tick_size )

        row = 5
        for j in range(self.num_mavs):
            axs[row].plot(t_real, (180/np.pi)*self.states[j, 5, :],linewidth=0.5)
        axs[row].set_xlabel('time(s)',loc='right', fontsize=alp_size)
        axs[row].set_ylabel('yaw (deg)', fontsize=alp_size-2)
        axs[row].tick_params(axis='x', labelsize=tick_size )
        axs[row].tick_params(axis='y', labelsize=tick_size )
        axs[row].set_yticks(np.arange(0,75,25))
        # axs[row].set_yticks(np.arange(-60,90,30))
    ...
    def plot_xyz(self):
        ...
        axs2.tick_params(axis='y', labelsize=tick_size,labelcolor='white')
        axs3.tick_params(axis='y', labelsize=tick_size,labelcolor='white')
        axs.legend(handles, labels, loc='upper right', fontsize=alp_size)
        fig.subplots_adjust(left=0.126, right=0.96,top=0.96,bottom=0.126, wspace=0.2,hspace=0.2)
        plt.show()
    ...
    def plot_rpy(self):
        ...
        axs.legend(handles, labels, loc='upper right', fontsize=26, handler_map={Line2D: HandlerLine2DWithWidth()})
        fig.subplots_adjust(left=0.126, right=0.96,top=0.96,bottom=0.126, wspace=0.2,hspace=0.2)
        plt.show()
