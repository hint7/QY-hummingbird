'''test the trained hht-RL model with GUI for escape'''
...
class testRLtrac():
    def __init__(self):
        ...
        self.trac_pos=np.array([1,0,0.5])
    ...
    def test(self):
        ...
        for i in range(whole_ctrl_steps):
            ...
            if(((i+up_pos)%up_pos)==0):
                ...
                self.trac_att = self.trac_att + np.array([0,0,(7.5/180)*np.pi])
            ...
            #     or i==(5*int(test_env.CTRL_FREQ)+3)
    ...
    def Circle_trac_xz(self,timefrac):
        ...
        # x = 2*R * np.cos(theta)-1.5*R * np.sin(theta)# x 坐标
        # y = self.initial_pos[1] # y 坐标
        # z = 0.32+1.1*R * np.sin(theta)+0.5*R * np.cos(theta) # z 坐标
        x = 1.8*R * np.cos(theta)-1.5*R * np.sin(theta)# x 坐标
        y = self.initial_pos[1] # y 坐标
        z = 0.32+1.1*R * np.sin(theta)+0.5*R * np.cos(theta) # z 坐标
        # x = 2*R * np.cos(theta)  # x 坐标
        # y = self.initial_pos[1] # y 坐标
        # z = 0.3+1.1*R * np.sin(theta) # z 坐标
        return np.array([x, y, z])
    ...
    def plot_xyz(self):
        ...
        axs.set_yticks(np.arange(-200, 1100, 100))
        axs2.set_yticks(np.arange(-200, 1100, 100))
        axs3.set_yticks(np.arange(-200,1100, 100))
