...
class RLattbody(RLMAV):
    ...
    def loadedge(self):
        '''create visual boundness'''
                # 创建黄色表面
        surface_visual_id = p.createVisualShape(
            shapeType=p.GEOM_BOX,  # 长方体几何
            halfExtents=[0.2, 0.2, 0.3],  # 长方体的半长度（长宽高的一半）
            rgbaColor=[1, 1, 0, 0.3]  # 黄色，不透明
        )
        # 定义红色边线
        line_thickness = 0.005  # 边线的厚度
        edge_positions = [
            # 顶面边线
            ([0, 0.2 + line_thickness, 0.8], [0.2, line_thickness, line_thickness]),  # 顶前横线
            ([0, -0.2 - line_thickness, 0.8], [0.2, line_thickness, line_thickness]),  # 顶后横线
            ([0.2 + line_thickness, 0, 0.8], [line_thickness, 0.2, line_thickness]),  # 顶右竖线
            ([-0.2 - line_thickness, 0, 0.8], [line_thickness, 0.2, line_thickness]),  # 顶左竖线

            # 底面边线
            ([0, 0.2 + line_thickness, 0.2], [0.2, line_thickness, line_thickness]),  # 底前横线
            ([0, -0.2 - line_thickness, 0.2], [0.2, line_thickness, line_thickness]),  # 底后横线
            ([0.2 + line_thickness, 0, 0.2], [line_thickness, 0.2, line_thickness]),  # 底右竖线
            ([-0.2 - line_thickness, 0, 0.2], [line_thickness, 0.2, line_thickness])   # 底左竖线
        ]

        # 创建黄色表面的主体
        p.createMultiBody(
            baseMass=0,
            baseVisualShapeIndex=surface_visual_id,
            basePosition=[0, 0, 0.5]  # 中心位置
        )

        # 创建红色边线的每一段
        for pos, size in edge_positions:
            edge_visual_id = p.createVisualShape(
                shapeType=p.GEOM_BOX,
                halfExtents=size,  # 每段边线的尺寸
                rgbaColor=[1, 0, 0, 1]  # 红色，不透明
            )
            p.createMultiBody(
                baseMass=0,
                baseVisualShapeIndex=edge_visual_id,
                basePosition=pos
            )
    ...
    def _computeTruncated(self):
        ...
        if self.trunc_flag:
            ...
            #  If flying too far
            if (abs(state[0]) > 0.2 or abs(state[1]) > 0.2 or abs(state[2]) >0.3 or
    ...
    def _housekeeping(self, p_this, seed0):
        ...
        yaw = np.pi / 72 * (2 * np.random.rand(1) - 1)
        random_att = 2* np.array([roll.item(), pitch.item(), yaw.item()])
        # random_att = np.array([0,0,0.5])
        # random_att = np.array([0,0,(30/180)*np.pi])
        # random_att = np.array([0,0,0])
        # random_att = np.array([0,0,(45/180)*np.pi])
        # random_pos = np.array([0,0,0.5])
    ...
    def _computeReward(self):
        ...
        state = self._getDroneStateVector()
        pos_e = np.linalg.norm(state[0:3])
        att_e = np.abs(state[3] )+ np.abs(state[4]) + 4* np.abs(state[5])
        vel_z_e = np.abs(state[8])
        w_e = np.abs(state[9])+np.abs(state[10])+ np.abs(state[11])
